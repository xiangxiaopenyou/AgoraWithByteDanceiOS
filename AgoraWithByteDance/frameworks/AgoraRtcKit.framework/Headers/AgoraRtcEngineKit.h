 ...
 * - false: Ignore, in which case this method does not sent the current video frame to the SDK.
 */
- (BOOL)onRenderVideoFrame:(AgoraOutputVideoFrame * _Nonnull)videoFrame uid:(NSUInteger)uid connectionId:(NSUInteger)connectionId;
- (BOOL)onTranscodedVideoFrame:(AgoraOutputVideoFrame * _Nonnull)videoFrame;


@optional
/**
 * Occurs each time the SDK receives a video frame captured by the MediaPlayer.
 *
 * After you successfully register the video frame observer, the SDK triggers this callback each time
 * a video frame is received. In this callback, you can get the video data captured by the MediaPlayer.
 * You can then pre-process the data according to your scenarios.
 *
 * After pre-processing, you can send the processed video data back to the SDK by setting the
 * `videoFrame` parameter in this callback.
 ...
 * - false: Ignore, in which case this method does not sent the current video frame to the SDK.
 */
- (BOOL)onMediaPlayerVideoFrame:(AgoraOutputVideoFrame * _Nonnull)videoFrame mediaPlayerId:(int)mediaPlayerId;
...
@note  Ensure that the size of the metadata that you specify in this callback does not exceed the value set in the maxMetadataSize callback.
 ...
 @param sourceType AgoraVideoSourceType
...
/** A specified audio effect playback stopped.
...
/** The statistics of the local video stream.
 ...
 Same as [localVideoStatBlock]([AgoraRtcEngineKit localVideoStatBlock:]). The SDK updates the application on the statistics of the local video stream once every two seconds.

 @param engine The AgoraRtcEngineKit object.
 @param stats  AgoraRtcLocalVideoStats
 */
...
/** The data stream received by the local user from the remote user within five seconds.
...
/** The data stream was not received by the local user from the remote user within five seconds.
...
/** The first frame of the remote user was decoded successfully. Same as [firstRemoteVideoDecodedBlock]([AgoraRtcEngineKit firstRemoteVideoDecodedBlock:]).
...
/** A remote video stream state changed.
 ...
 * @param elapsed The time elapsed (ms) from the local user calling ref "joinChannel" until this callback is triggered.
 */
 ...
 *
 * This callback reports the statistics more closely linked to the real-user
 * experience of the video transmission quality.
 *
 * Schemes such as FEC (Forward Error Correction) or retransmission counter the
 * frame loss rate. Hence, users may find the overall video quality acceptable
 * even when the packet loss rate is high.
 * @param engine The AgoraRtcEngineKit object.
...
/**
 * Initializes the Agora SDK service with config.
 *
 * After the initialization, the service is set to enable audio by default.
 @param config    RtcEngine config info.
 @param delegate `AgoraRtcEngineDelegate`.
 @return An `AgoraRtcEngineKit` object.
 */
 ...
 * @param profile  The sample rate, bitrate, encoding mode, and the number of
 * channels, see #AgoraAudioProfile.
 * @param scenario The audio application scenarios, see #AgoraAudioScenario.
 *
 * @return
 * - 0: Success.
 * - <0: Failure.
 ...
 * - < 0: Failure.
 */
- (int)enableExtensionWithVendor:(AgoraVideoSourceType)sourceType vendor:(NSString * __nonnull)vendor enabled:(BOOL)enabled;
 ...
 * - < 0: Failure.
 */
- (int)setExtensionPropertyWithVendor:(AgoraVideoSourceType)sourceType vendor:(NSString * __nonnull)vendor key:(NSString * __nonnull)key value:(NSString * __nonnull)value;
...
/** 
 * @deprecated
 * Configures the remote video display mode
 ...
 * {@link setAudioProfile:scenario: setAudioProfile} as `AgoraAudioProfileMusicHighQuality`(4) or `AgoraAudioProfileMusicHighQualityStereo`(5).
 ...
 * {@link setAudioProfile:scenario: setAudioProfile} as `AgoraAudioProfileMusicHighQuality`(4) or `AgoraAudioProfileMusicHighQualityStereo`(5). Otherwise, this method call does not take effect.
 * - This method works best with the human voice, and Agora does not recommend using it for audio containing music and a human voice.
 ...
 *
 * A successful method call triggers the {@link AgoraRtcEngineDelegate.rtcEngine:audioMixingStateChanged:errorCode: audioMixingStateChanged} callback on the local client to report the `AgoraAudioMixingStateTypePlaying` state.
 *
...
- (int)startAudioRecording:(AgoraAudioFileRecordingConfig * _Nonnull)config;
 ...
 * - `YES`: The external video source is encoded.
 * - `NO`: The external video source is not encoded.
 */
- (void)setExternalVideoSource:(BOOL)enable useTexture:(BOOL)useTexture encodedFrame:(BOOL)encodedFrame;
...
- (int)pushExternalAudioFrameRawData:(void * _Nonnull)data
                            ...
                            sourceId:(NSInteger)sourceId
                           timestamp:(NSTimeInterval)timestamp __deprecated_msg("pushExternalAudioFrameNSData:sourceId:timestamp instead.");
...
/** Creates a data stream.
 ...
 @note  Set reliable and ordered both as YES or both as NO. Do not set one as YES and the other as NO.

 @param streamId Stream ID
 @param reliable * YES: The recipients will receive data from the sender within 5 seconds. If the recipient does not receive the sent data within 5 seconds, the data channel will report an error to the application.
 * NO: The recipients will not receive any data, and it will not report any error upon data missing.

 @param ordered  * YES: The recipients will receive data in the order of the sender.
 * NO: The recipients will not receive data in the order of the sender.
...
/** Sends data stream messages to all users in a channel.
 ...
 The API controls the data channel transfer rate.
 Each client can send up to 6 kB of data per second. Each user can have up to five data channels simultaneously.

 @param streamId Stream ID.
 @param message  Data to send.

 @return * 0: Success.
 * <0: Failure, such as AgoraErrorCodeSizeTooLarge(-114)/AgoraErrorCodeTooOften(-12)/AgoraErrorCodeBitrateLimit(-115).
*/
 ...
 *
 * For example, if you set the log level to WARNING, you see the logs within
 * levels CRITICAL, ERROR, and WARNING.
...
/** The first remote video frame is displayed on the screen.
 @deprecated From v1.1
...
/** A user's video stream has paused/resumed.
 @deprecated From v1.1

 @param uid   User ID.
 @param muted - YES: Paused.
 - NO: Resumed.
 */
...
#pragma mark - String uid for AgoraRtcEngineKit

@interface AgoraRtcEngineKit (StringUID)
...
- (int)setRemoteRenderMode2:(NSString * _Nonnull)userId
                       mode:(AgoraVideoRenderMode) mode;

- (int)switchView2:(NSString * _Nonnull)userId1
        andAnother:(NSString * _Nonnull)userId2 __deprecated;

- (int)muteRemoteAudioStream2:(NSString * _Nonnull)userId mute:(BOOL)mute;

- (int)setRemoteVideoStream2:(NSString * _Nonnull)userId
                        type:(AgoraVideoStreamType)streamType;

- (int)muteRemoteVideoStream2:(NSString * _Nonnull)userId
                         mute:(BOOL)mute;

- (int)sendPublishingRequestToOwner2:(NSString * _Nonnull) userId;
- (int)answerPublishingRequestOfUid2:(NSString * _Nonnull) userId accepted:(bool)accepted;
- (int)sendUnpublishingRequestToUid2:(NSString * _Nonnull) userId;
